...
class UpdateRestaurantTest extends TestCase
{
    ...
    public function test_admin_updates_restaurant_name_and_legal_name(): void
    {
        $restaurant = EloquentRestaurant::factory()->create([
            'name' => 'Old Name',
            'legal_name' => 'Old Name SL',
        ]);
    ...
    public function test_returns_409_when_email_already_taken(): void
    {
        ...
        $token = $this->issueToken(
            userId: Uuid::create($admin->uuid),
            restaurantId: Uuid::create($restaurantB->uuid),
            role: UserRole::admin(),
        );
