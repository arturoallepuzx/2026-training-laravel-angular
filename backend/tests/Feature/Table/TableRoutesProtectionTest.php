...
class TableRoutesProtectionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_get_all_returns_401_without_token(): void
    {
        ...
        $response = $this->getJson("/api/restaurants/{$restaurant->uuid}/tables");
    ...
    public function test_get_all_returns_401_when_token_is_for_different_restaurant(): void
    {
        ...
        $response = $this->getJson(
            "/api/restaurants/{$restaurantB->uuid}/tables",
            ['Authorization' => 'Bearer '.$token],
        );
    ...
    public function test_post_returns_403_with_operator_token(): void
    {
        ...
        $response = $this->postJson(
            "/api/restaurants/{$restaurant->uuid}/tables",
            ['zone_id' => Uuid::generate()->value(), 'name' => 'Mesa 1'],
            ['Authorization' => 'Bearer '.$token],
        );
    ...
    public function test_put_returns_403_with_operator_token(): void
    {
        ...
        $response = $this->putJson(
            "/api/restaurants/{$restaurant->uuid}/tables/".Uuid::generate()->value(),
            ['name' => 'Mesa 1'],
            ['Authorization' => 'Bearer '.$token],
        );
